...
export let commands = {
  ...
  undo: cmb => {
    if(cmb.history.done.length > 0) {
      cmb.say("undo " + cmb.history.done[0].announcement);
      cmb.cm.execCommand("undo");
      cmb.history.undone.unshift(cmb.history.done.shift());
    }
    else { playSound(BEEP); }
    return true;
  },
  redo: cmb => {
    if(cmb.history.undone.length > 0) {
      cmb.say("redo " + cmb.history.undone[0].announcement);
      cmb.cm.execCommand("redo");
      cmb.history.done.unshift(cmb.history.undone.shift());
    }
    else { playSound(BEEP); }
    return true;
  },
