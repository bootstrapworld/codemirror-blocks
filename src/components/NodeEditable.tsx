...
const NodeEditable = (props: Props) => {
  ...
  const { initialValue, isErrored } = useSelector((state: RootState) => {
    ...
    const initialValue =
      props.value === null ? props.target.getText(state.ast, SHARED.cm) : "";
  ...
  const onBlur = (e: React.SyntheticEvent) => {
    ...
    dispatch((dispatch: AppDispatch, getState: () => RootState) => {
      ...
      if (value === initialValue || !value) {
        ...
        dispatch(activateByNid(SHARED.cm, nid));
        return;
      }

      let annt = `${props.isInsertion ? "inserted" : "changed"} ${value}`;
      const onSuccess = () => {
        dispatch(activateByNid(SHARED.cm, null, { allowMove: false }));
        props.onChange(null);
        props.onDisableEditable();
        setErrorId("");
        say(annt);
      ...
      insert(value, target, SHARED.cm, onSuccess, onError, annt);
    });
  };
