import * as cycle from "./cyclicJSON.js";
  ...
  RuleFactory.make = function(name, symbols, lookahead, pos) {
    ...
    var symbolsStr = "" + symbols.map((s) => s.asString);
    if (findByName) {
      var findBySymbols = findByName[symbolsStr];
      if (findBySymbols) {
        ...
        findByName[symbolsStr] = byLookahead;
        return rule;
      }
    } else {
      var rule = new Rule(name, symbols, lookahead, pos);
      ...
      var byPos = {}; byPos[pos] = rule;
      var byLookahead = {}; byLookahead[lookahead ? lookahead.asString : undefined] = byPos;
      var bySymbols = {}; bySymbols[symbolsStr] = byLookahead;
      ...
      for (var j = 0; j < g.atoms.size(); j++) {
        ...
        if (atom in tableRow) {
          ...
          if (tr[0] >= 0)
            nr.push = tr[0];
          var last = tr.length;
          if (tr.length > 1 && tr[tr.length - 1] === -1) {
            last--;
            nr.accept = true;
          }
  ...
  Grammar.prototype = {
    ...
    computeFirstOfStrings: function(string) {
      var joinedStrings = "" + string.map((s) => s.asString);
      var ret = this.firstStringCache[joinedStrings];
      if (ret !== undefined) return ret;
      ret = this.firstStringCache[joinedStrings] = new KeyedSet("key");
      var nullable = true;
      ...
      if (token_source.isEmpty()) {
        ...
        // try {
          while (hasNext && this.U[i].size() > 0) {
            ...
            var next_tok = hasNext ? token_source.next() : undefined;
      ...
      for (var name in this.rules) {
        ...
        for (var i = 0; i < rules_name.length; i++) {
          ...
          for (var j = 1/*note: not 0*/; j < rule.symbols.length; j++) {
            ...
            var sliceStr = "" + sliceStrs;
            if (this.computeFirstOfStrings(slice).contains(EPSILON) && this.I[sliceStr] === undefined) {
              var eSPPF_slice = new SPPFNode(sliceStr, null);
