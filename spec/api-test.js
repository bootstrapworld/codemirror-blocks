...
describe("when testing CM apis,", function () {
  ...
  it("those that simply pass through should not throw errors", async function () {
    ...
    expect(() =>
      this.cmb.removeLineClass(lineNumber, code, className)
    ).not.toThrow();
    expect(() => this.cmb.scrollIntoView()).not.toThrow();
    expect(() => this.cmb.scrollTo()).not.toThrow();
    expect(() =>
      this.cmb.setGutterMarker(lineNumber, marker, domNode)
    ).not.toThrow();
  ...
  it("getCursor should work as-is for Text", async function () {
    ...
    expect(simpleCursor(this.cmb.getCursor("head"))).toEqual({
      line: 0,
      ch: 7,
    });
    expect(simpleCursor(this.cmb.getCursor("anchor"))).toEqual({
      line: 0,
      ch: 0,
    });
    expect(simpleCursor(this.cmb.getCursor("from"))).toEqual({
      line: 0,
      ch: 0,
    });
  ...
  it("getSelection should work as-expected for blocks", async function () {
    ...
    await finishRender(this.cmb);
    expect(this.currentFirstRoot().element.getAttribute("aria-selected")).toBe(
      "true"
    );
  });
  ...
  it("setBookmark", async function () {
    ...
    await finishRender(this.cmb);
    expect(() =>
      this.cmb.setBookmark({ line: 0, ch: 2 }, { widget: domNode })
    ).toThrow();
    this.cmb.setBlockMode(false);
    await finishRender(this.cmb);
    expect(
