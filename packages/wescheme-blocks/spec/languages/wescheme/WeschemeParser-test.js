...
describe("The WeScheme Parser,", function () {
  ...
  describe("when parsing comments,", function () {
    ...
    it("should keep track of the text of the comment", function () {
      expect(this.ast.rootNodes[0].options.comment.comment).toBe(
        "this is a comment"
      );
    });
  });
  ...
  describe("when parsing let-like expressions,", function () {
    ...
    it("should get the correct aria-label", function () {
      expect(this.ast.rootNodes[0].options["aria-label"]).toBe(
    ...
    it("should convert the bindings to a sequence, correctly", function () {
      ...
      expect(this.ast.rootNodes[0].bindings.exprs[0].type).toBe(
        "variableDefinition"
      );
      expect(this.ast.rootNodes[0].bindings.exprs[0].name.value).toBe("x");
      expect(this.ast.rootNodes[0].bindings.exprs[1].type).toBe(
        "variableDefinition"
      );
      expect(this.ast.rootNodes[0].bindings.exprs[1].name.value).toBe("y");
      expect(this.ast.rootNodes[0].bindings.exprs[2].type).toBe(
        "variableDefinition"
      );
  ...
  describe("when setting aria-labels", function () {
    it("should make symbols, and numbers be set to themselves", function () {
      expect(this.parser.parse("1").rootNodes[0].options["aria-label"]).toBe(
        "1"
      );
      expect(
        this.parser.parse("symbol").rootNodes[0].options["aria-label"]
      ).toBe("symbol");
    });

    it("should make boolean values be set to 'true' or 'false'", function () {
      expect(this.parser.parse("#t").rootNodes[0].options["aria-label"]).toBe(
        "true, a Boolean"
      );
    });
    ...
    it("should make string values be set to 'string '+the contents of the string", function () {
      expect(
        this.parser.parse('"hello"').rootNodes[0].options["aria-label"]
      ).toBe("hello, a String");
    });

    it("should make expression (print 'hello') into 'print expression, 1 input'", function () {
      expect(
        this.parser.parse('(print "hello")').rootNodes[0].options["aria-label"]
      ).toBe("print expression, 1 input");
      expect(
        this.parser.parse('(print "hello" "world")').rootNodes[0].options[
          "aria-label"
        ]
    ...
    it("should make and/or expressions just like regular expressions", function () {
      expect(
        this.parser.parse("(and true true)").rootNodes[0].options["aria-label"]
      ).toBe("and expression, 2 inputs");
      expect(
        this.parser.parse("(or false true)").rootNodes[0].options["aria-label"]
      ).toBe("or expression, 2 inputs");
    });

    it("should turn symbols into readable words", function () {
      expect(
        this.parser.parse("(* 1 2)").rootNodes[0].options["aria-label"]
      ).toBe("multiply expression, 2 inputs");
      expect(
        this.parser.parse("(/ 1 2)").rootNodes[0].options["aria-label"]
      ).toBe("divide expression, 2 inputs");
      expect(
        this.parser.parse("(foo? 0)").rootNodes[0].options["aria-label"]
      ).toBe("foo-huh expression, 1 input");
      expect(
        this.parser.parse("(set! x 2)").rootNodes[0].options["aria-label"]
